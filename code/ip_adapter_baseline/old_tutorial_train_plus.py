"""
Замените этим файлом аналогичный из IP-adapter.
Изменения:
- настроено логгирование в wandb
- запуск осуществляется из скрипта train.sh
"""
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        wandb.init(
            project="ip-adapter",
            dir=args.output_dir,
            name=f"ip-adapter-{datetime.now().strftime('%Y%m%d-%H%M%S')}",
        )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                if accelerator.is_main_process:
                    ...
                    accelerator.log(
                        {"train_loss": avg_loss, "epoch": epoch, "step": global_step},
                        step=global_step,
                    )

            global_step += 1

            if (global_step % args.save_steps == 0) and (avg_loss < best_loss):
